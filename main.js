...
class HumanityClient {
  ...
  async loadWallets() {
    ...
    if (proxies.length !== privateKeys.length) {
      throw new Error("The number of proxies does not match the number of accounts.");
    }
    ...
    return privateKeys.map((privateKey, index) => {
      ...
      const proxy = `socks5://${proxies[index]}`;
      ...
      return { address: account.address, privateKey: formattedKey, axios: axiosInstance };
    });
  }
